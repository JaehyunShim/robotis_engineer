...
/* Authors: SCH, Jay Song, Kayman, Ryan Shim */

#ifndef ROBOTIS_ENGINEER_KDL_H_
#define ROBOTIS_ENGINEER_KDL_H_

#include <vector>
#include <eigen3/Eigen/Eigen>

#include "robotis_engineer_kdl_define.h"
#include "link_data.h"
...
class EngineerKinematicsDynamics
{

 public:
  EngineerKinematicsDynamics();
  ~EngineerKinematicsDynamics();
  EngineerKinematicsDynamics(TreeSelect tree);

  std::vector<int> findRoute(int to);
  std::vector<int> findRoute(int from, int to);

  double calcTotalMass(int joint_id);
  Eigen::MatrixXd calcMC(int joint_id);   // MC stands for the sum of mass * center of mass of each link
  ...
  bool calcInverseKinematics(int to, Eigen::MatrixXd tar_position, Eigen::MatrixXd tar_orientation, int max_iter,
                             double ik_err);        // IK calculating from the base
  bool calcInverseKinematics(int from, int to, Eigen::MatrixXd tar_position, Eigen::MatrixXd tar_orientation,
                             int max_iter, double ik_err);    // IK calculating from a certian joinn

  // with weight
  bool calcInverseKinematics(int to, Eigen::MatrixXd tar_position, Eigen::MatrixXd tar_orientation,
                             int max_iter, double ik_err, Eigen::MatrixXd weight);    // Considering weight and damping??
  bool calcInverseKinematics(int from, int to, Eigen::MatrixXd tar_position, Eigen::MatrixXd tar_orientation,
                             int max_iter, double ik_err, Eigen::MatrixXd weight);
  // The below three are for walking.
  bool computeIK(double *out, double x, double y, double z,                                                 
                                   double roll, double pitch, double yaw);
  bool calcInverseKinematicsForLeg(double *out, double x, double y, double z,                                                 
                                   double roll, double pitch, double yaw);
  bool calcInverseKinematicsForRightLeg(double *out, double x, double y, double z, 
                                        double roll, double pitch, double yaw);
  bool calcInverseKinematicsForLeftLeg(double *out, double x, double y, double z,
                                       double roll, double pitch, double yaw);

  LinkData *robotis_engineer_link_data_[ ALL_JOINT_ID + 1];
