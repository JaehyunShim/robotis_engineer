...
OnlineWalkingModule::OnlineWalkingModule()
  : control_cycle_sec_(0.008),
    ...
    walking_phase_(DSP),
    total_mass_(3.5),     // modify here later....
    foot_distance_(0.0705)    
{
  enable_       = false;
  ...
  robotis_engineer_kdl_ = new EngineerKinematics();
  ...
  /* leg */
  joint_name_to_id_["r_hip_roll"] = 1;
  joint_name_to_id_["r_hip_pitch"] = 2;
  joint_name_to_id_["r_ankle_pitch"] = 3;
  joint_name_to_id_["r_ankle_roll"] = 4;

  joint_name_to_id_["l_hip_roll"] = 5;
  joint_name_to_id_["l_hip_pitch"] = 6;
  joint_name_to_id_["l_ankle_pitch"] = 7;
  joint_name_to_id_["l_ankle_roll"] = 8;
  ...
  des_joint_accel_.resize(number_of_joints_, 0.0);        // Deisred Joint Acceleration
  ...
  des_body_Q_.resize(4, 0.0);        // Q stands for what..? angle???
  ...
  x_lipm_.resize(3, 0.0);                  // linear inverted pendulum model
  ...
  walking_param_.foot_height_max = 0.020; 
...
void OnlineWalkingModule::queueThread()
{
  ...
  // About Path Planning Walking... I guess..
...
// SET THE INIT POSE HERE
void OnlineWalkingModule::resetBodyPose()
{
  // maybe COM??
  des_body_pos_[0] = 0.0;       
  des_body_pos_[1] = 0.0;
  des_body_pos_[2] = 0.22; // should be when bending legs or stretching?
...
void OnlineWalkingModule::parseJointFeedforwardGainData(const std::string &path)
{
  ...
  joint_feedforward_gain_[joint_name_to_id_["r_ankle_pitch"]-1] = doc["r_ankle_pitch_gain"].as<double>();
  joint_feedforward_gain_[joint_name_to_id_["r_ankle_roll"]-1]  = doc["r_ankle_roll_gain"].as<double>();
  ...
  joint_feedforward_gain_[joint_name_to_id_["l_ankle_pitch"]-1] = doc["l_ankle_pitch_gain"].as<double>();
  joint_feedforward_gain_[joint_name_to_id_["l_ankle_roll"]-1]  = doc["l_ankle_roll_gain"].as<double>();
}
...
void OnlineWalkingModule::walkingParamCallback(const robotis_engineer_online_walking_module_msgs::WalkingParam& msg)
{
  walking_param_ = msg;
}

void OnlineWalkingModule::goalJointPoseCallback(const robotis_engineer_online_walking_module_msgs::JointPose& msg)
{
  if (enable_ == false)
    return;
...
void OnlineWalkingModule::goalKinematicsPoseCallback(const robotis_engineer_online_walking_module_msgs::KinematicsPose& msg)
{
  if (enable_ == false)
    return;
...
void OnlineWalkingModule::footStep2DCallback(const robotis_engineer_online_walking_module_msgs::Step2DArray& msg)
{
  if (enable_ == false)
    return;
  ...
  robotis_engineer_online_walking_module_msgs::Step2DArray foot_step_msg;

  int old_size = msg.footsteps_2d.size();
  int new_size = old_size + 3;

  robotis_engineer_online_walking_module_msgs::Step2D first_msg;
  robotis_engineer_online_walking_module_msgs::Step2D second_msg;
  ...
  if (control_type_ == NONE || control_type_ == WALKING_CONTROL)
  {
    ...
    {
      robotis_engineer_online_walking_module_msgs::Step2D step_msg = msg.footsteps_2d[i];
    ...
    robotis_engineer_online_walking_module_msgs::Step2D step_msg = msg.footsteps_2d[old_size-1];
...
void OnlineWalkingModule::footStepCommandCallback(const robotis_engineer_online_walking_module_msgs::FootStepCommand& msg)
{
  if (enable_ == false)
    return;
...
//   // Forward Kinematics
//   robotis_engineer_kdl_->initialize(des_body_pos, des_body_rot);

//   Eigen::VectorXd r_leg_joint_pos, l_leg_joint_pos;

//   r_leg_joint_pos.resize(4);
//   r_leg_joint_pos(0) = des_joint_pos_[joint_name_to_id_["r_hip_roll"]-1];
//   r_leg_joint_pos(1) = des_joint_pos_[joint_name_to_id_["r_hip_pitch"]-1];
//   r_leg_joint_pos(2) = des_joint_pos_[joint_name_to_id_["r_ankle_pitch"]-1];
//   r_leg_joint_pos(3) = des_joint_pos_[joint_name_to_id_["r_ankle_roll"]-1];

//   l_leg_joint_pos.resize(4);
//   l_leg_joint_pos(0) = des_joint_pos_[joint_name_to_id_["l_hip_roll"]-1];
//   l_leg_joint_pos(1) = des_joint_pos_[joint_name_to_id_["l_hip_pitch"]-1];
...
//   robotis_engineer_kdl_->setJointPosition(r_leg_joint_pos, l_leg_joint_pos);
...
//   robotis_engineer_kdl_->solveForwardKinematics(r_leg_pos, r_leg_Q,
//                                    l_leg_pos, l_leg_Q);
...
//   robotis_engineer_kdl_->finalize();
// }
...
bool OnlineWalkingModule::setBalanceControl()
{
  ...
  // ======= ======= //
  robotis_engineer_kdl_->initialize(des_body_pos_mod, des_body_rot_mod);
  ...
  robotis_engineer_kdl_->setJointPosition(r_leg_joint_pos, l_leg_joint_pos);   // cuz this kdl method uses Jacobian??
  ...
  ik_success = robotis_engineer_kdl_->solveInverseKinematics(r_leg_output,
...
//// Use the below three parameters and pre-built Humanoid kdl and calc des_joint_pos
  ...
  double bodyToFirstJoint_width  = 0.03525;
  Eigen::MatrixXd firstjoint_right = body_pose_mod.block<3,1>(0,3);
  Eigen::MatrixXd firstjoint_left = body_pose_mod.block<3,1>(0,3);

  firstjoint_right(1) -= bodyToFirstJoint_width;
  firstjoint_right(2) -= bodyToFirstJoint_height;

  firstjoint_left(1)  += bodyToFirstJoint_width;
  firstjoint_left(2)  -= bodyToFirstJoint_height;

  des_r_foot_pos_mod -= firstjoint_right;
  des_l_foot_pos_mod -= firstjoint_left;
// des_body_pos_mod, des_body_rot_mod);
// des_r_foot_pos_mod,des_r_foot_Q_mod,
// des_l_foot_pos_mod,des_l_foot_Q_mod);
  ...
  if (ik_success == true)
  {
    ...
    des_joint_pos_[joint_name_to_id_["r_ankle_pitch"]-1]  = r_leg_output[3];
    des_joint_pos_[joint_name_to_id_["r_ankle_roll"]-1]   = r_leg_output[4];
    ...
    des_joint_pos_[joint_name_to_id_["l_ankle_pitch"]-1]  = l_leg_output[3];
    des_joint_pos_[joint_name_to_id_["l_ankle_roll"]-1]   = l_leg_output[4];
  }
...
void OnlineWalkingModule::setFeedforwardControl()
{
  ...
  if (walking_leg_ == LEFT_LEG)
  {
    ...
    support_leg_gain[joint_name_to_id_["r_ankle_pitch"]-1] = 1.0;
    support_leg_gain[joint_name_to_id_["r_ankle_roll"]-1]  = 1.0;

    support_leg_gain[joint_name_to_id_["l_hip_roll"]-1]    = 0.0;
    support_leg_gain[joint_name_to_id_["l_hip_pitch"]-1]   = 0.0;
    support_leg_gain[joint_name_to_id_["l_ankle_pitch"]-1] = 0.0;
    support_leg_gain[joint_name_to_id_["l_ankle_roll"]-1]  = 0.0;
  }
  else if (walking_leg_ == RIGHT_LEG)
  {
    support_leg_gain[joint_name_to_id_["r_hip_roll"]-1]    = 0.0;
    support_leg_gain[joint_name_to_id_["r_hip_pitch"]-1]   = 0.0;
    support_leg_gain[joint_name_to_id_["r_ankle_pitch"]-1] = 0.0;
    support_leg_gain[joint_name_to_id_["r_ankle_roll"]-1]  = 0.0;

    support_leg_gain[joint_name_to_id_["l_hip_roll"]-1]    = 1.0;
    support_leg_gain[joint_name_to_id_["l_hip_pitch"]-1]   = 1.0;
    support_leg_gain[joint_name_to_id_["l_ankle_pitch"]-1] = 1.0;
    support_leg_gain[joint_name_to_id_["l_ankle_roll"]-1]  = 1.0;
  }
                              ...
                              std::map<std::string, double> sensors)
{
  ...
  pelvis_pose_msg.pose.position.z = des_body_pos_[2] - 0.0907;   // for what...?
...
bool OnlineWalkingModule::getJointPoseCallback(robotis_engineer_online_walking_module_msgs::GetJointPose::Request &req,
                                           robotis_engineer_online_walking_module_msgs::GetJointPose::Response &res)
{
  for (int i=0; i<number_of_joints_; i++)
  {
...
bool OnlineWalkingModule::getKinematicsPoseCallback(robotis_engineer_online_walking_module_msgs::GetKinematicsPose::Request &req,
                                                robotis_engineer_online_walking_module_msgs::GetKinematicsPose::Response &res)
{
...
bool OnlineWalkingModule::definePreviewMatrix()      // used what to compute the below ???
{
