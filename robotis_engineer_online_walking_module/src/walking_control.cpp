...
#include <stdio.h>                                                // dynamics filter using preview control....
#include "robotis_engineer_online_walking_module/walking_control.h"
    ...
    walking_phase_(PHASE_COUNT)
{
  ...
  foot_size_x_ = 0.127;          // <-- sure..?? not for THORMANG??
  foot_size_y_ = 0.0125;
...
void WalkingControl::initialize(robotis_engineer_online_walking_module_msgs::FootStepCommand foot_step_command,
                                ...
                                std::vector<double_t> init_l_foot_pos, std::vector<double_t> init_l_foot_Q)
{
  ...
  Eigen::Quaterniond body_Q(init_body_Q[3],init_body_Q[0],init_body_Q[1],init_body_Q[2]);   // 0,1,2 for rpy, 3 for the orientation for the specified rpy.
  init_body_Q_ = body_Q;
  des_body_Q_ = body_Q;
...
void WalkingControl::initialize(robotis_engineer_online_walking_module_msgs::Step2DArray foot_step_2d,
                                std::vector<double_t> init_body_pos, std::vector<double_t> init_body_Q,
                                std::vector<double_t> init_r_foot_pos, std::vector<double_t> init_r_foot_Q,
                                std::vector<double_t> init_l_foot_pos, std::vector<double_t> init_l_foot_Q)
{
...
void WalkingControl::next()      // why named like this ??
{
...
void WalkingControl::set(double time, int step, bool foot_step_2d)
{
  ...
  calcFootStepPose(time,step);         // Calculate Foot Step Position??
  calcRefZMP(step);                    // Calculate Reference ZMP considering the foot step pose
  calcPreviewControl(time,step);       // Calculate Reference-ZMP-Considered COM position

  // /* ----- Inverse Kinematics ---- */
  double dsp_length = 0.5*(fin_time_ - init_time_)*dsp_ratio_;  // Double stance phase
...
void WalkingControl::transformFootStep2D()
{
  ...
  robotis_engineer_online_walking_module_msgs::FootStepArray foot_step_param;

  for (int step=0; step<foot_step_size_; step++)
  {
    robotis_engineer_online_walking_module_msgs::Step2D msg = foot_step_2d_.footsteps_2d[step];
...
void WalkingControl::calcRefZMP(int step)    // Calculate Reference ZMP
{
  if (step == 0 || step == 1)
  {
...
void WalkingControl::calcPreviewParam(std::vector<double_t> K, int K_row, int K_col,   // page 146!!!
                                      std::vector<double_t> P, int P_row, int P_col)   // for the initial walking
{
  //
//  ROS_INFO("lipm_height_ : %f", lipm_height_);
  ...
  delete preview_control_;   // why delete...???
}

void WalkingControl::calcPreviewControl(double time, int step)    // For long-term walking  refer to page 147
{
  int index_new;
  ...
  for (int i=0; i<preview_size_; i++)
  {
    ...
    // The third term of the right hand side of (4.78)
  ...
  // return the below three parameters
