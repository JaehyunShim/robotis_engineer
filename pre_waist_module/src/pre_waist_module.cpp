...
#include <stdio.h>
#include "pre_waist_module/pre_waist_module.h"
...
{
  enable_ = false;
  module_name_ = "waist_control_module";
  control_mode_ = robotis_framework::PositionControl;

  result_["waist_pan"] = new robotis_framework::DynamixelState();
  result_["waist_tilt"] = new robotis_framework::DynamixelState();

  using_joint_name_["waist_pan"] = 0;
  using_joint_name_["waist_tilt"] = 1;

  max_angle_[using_joint_name_["waist_pan"]] = 90 * DEGREE2RADIAN;
  min_angle_[using_joint_name_["waist_pan"]] = -90 * DEGREE2RADIAN;
  max_angle_[using_joint_name_["waist_tilt"]] = 0 * DEGREE2RADIAN;
  min_angle_[using_joint_name_["waist_tilt"]] = -45 * DEGREE2RADIAN;
...
void WaistControlModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ros::NodeHandle param_nh("~");
  angle_unit_ = param_nh.param("angle_unit", 35.0);

  ROS_WARN_STREAM("Waist control - angle unit : " << angle_unit_);

  queue_thread_ = boost::thread(boost::bind(&WaistControlModule::queueThread, this));
...
void WaistControlModule::queueThread()
{
  ros::NodeHandle ros_node;
  ...
  /* subscribe topics */
  ros::Subscriber set_waist_joint_sub = ros_node.subscribe("/robotis/waist_control/set_joint_states", 1,
                                                          &WaistControlModule::setWaistJointCallback, this);
  ros::Subscriber set_waist_joint_offset_sub = ros_node.subscribe("/robotis/waist_control/set_joint_states_offset", 1,
                                                                 &WaistControlModule::setWaistJointOffsetCallback, this);
  ros::Subscriber set_waist_scan_sub = ros_node.subscribe("/robotis/waist_control/scan_command", 1,
                                                         &WaistControlModule::setWaistScanCallback, this);
...
void WaistControlModule::setWaistJointCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  setWaistJoint(msg, false);
}

void WaistControlModule::setWaistJointOffsetCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  setWaistJoint(msg, true);
}

void WaistControlModule::setWaistJoint(const sensor_msgs::JointState::ConstPtr &msg, bool is_offset)
{
  if (enable_ == false)
  {
    ROS_INFO_THROTTLE(1, "Waist module is not enable.");
  ...
  // generate trajectory
  tra_gene_thread_ = new boost::thread(boost::bind(&WaistControlModule::jointTraGeneThread, this));
  delete tra_gene_thread_;
}

void WaistControlModule::setWaistScanCallback(const std_msgs::String::ConstPtr &msg)
{
  if (enable_ == false)
  {
    ROS_ERROR_THROTTLE(1, "Waist control module is not enabled, scan command is canceled.");
    return;
  }
  else
...
bool WaistControlModule::checkAngleLimit(const int joint_index, double &goal_position)
{
...
void WaistControlModule::process(std::map<std::string, robotis_framework::Dynamixel *> dxls,
...
bool WaistControlModule::isRunning()
{
  return is_moving_;
}

void WaistControlModule::onModuleEnable()
{
  scan_state_ = NoScan;
}

void WaistControlModule::onModuleDisable()
{
  // init value
  ...
  std::cout << "waist_control_module : disable";
}

void WaistControlModule::startMoving()
{
  is_moving_ = true;
...
void WaistControlModule::finishMoving()
{
  // init value
  calc_joint_tra_ = goal_position_;
  tra_size_ = 0;
  ...
  // log
  publishStatusMsg(robotis_controller_msgs::StatusMsg::STATUS_INFO, "Waist movement is finished.");
...
void WaistControlModule::stopMoving()
{
  // init value
  calc_joint_tra_ = goal_position_;
  tra_size_ = 0;
...
void WaistControlModule::generateScanTra(const int waist_direction)
{
  switch (waist_direction)
  {
  case TopLeft:
  {
    target_position_.coeffRef(0, using_joint_name_["waist_pan"]) =  min_angle_[using_joint_name_["waist_pan"]] * 0.6;
    target_position_.coeffRef(0, using_joint_name_["waist_tilt"]) = min_angle_[using_joint_name_["waist_tilt"]] * 0.25;
    break;
  }

  case TopRight:
  {
    target_position_.coeffRef(0, using_joint_name_["waist_pan"]) =  max_angle_[using_joint_name_["waist_pan"]] * 0.6;
    target_position_.coeffRef(0, using_joint_name_["waist_tilt"]) = min_angle_[using_joint_name_["waist_tilt"]] * 0.25;
    break;
  }
  ...
  case BottomLeft:
  {
    target_position_.coeffRef(0, using_joint_name_["waist_pan"]) = min_angle_[using_joint_name_["waist_pan"]] * 0.45;
    target_position_.coeffRef(0, using_joint_name_["waist_tilt"]) = min_angle_[using_joint_name_["waist_tilt"]] * 0.8;
    break;
  }

  case BottomRight:
  {
    target_position_.coeffRef(0, using_joint_name_["waist_pan"]) = max_angle_[using_joint_name_["waist_pan"]] * 0.45;
    target_position_.coeffRef(0, using_joint_name_["waist_tilt"]) = min_angle_[using_joint_name_["waist_tilt"]] * 0.8;
    break;
  }
 ...
 */
Eigen::MatrixXd WaistControlModule::calcMinimumJerkTraPVA(double pos_start, double vel_start, double accel_start,
...
void WaistControlModule::jointTraGeneThread()
{
  tra_lock_.lock();
...
void WaistControlModule::publishStatusMsg(unsigned int type, std::string msg)
{
  ...
  status_msg.module_name = "Waist Control";
