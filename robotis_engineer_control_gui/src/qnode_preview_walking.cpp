...
#include "../include/robotis_engineer_control_gui/qnode.hpp"
...
{
  // preview walking
  foot_step_command_pub_ = ros_node.advertise<robotis_engineer_online_walking_module_msgs::FootStepCommand>("/robotis/online_walking/foot_step_command", 0);
  walking_param_pub_ = ros_node.advertise<robotis_engineer_online_walking_module_msgs::WalkingParam>("/robotis/online_walking/walking_param", 0);
  set_walking_footsteps_pub_ = ros_node.advertise<robotis_engineer_online_walking_module_msgs::Step2DArray>(
  ...
  joint_pose_msg_pub_ = ros_node.advertise<robotis_engineer_online_walking_module_msgs::JointPose>("/robotis/online_walking/goal_joint_pose", 0);
...
void QNode::setWalkingFootsteps(const double &step_time)
{
  ...
  robotis_engineer_online_walking_module_msgs::Step2DArray footsteps;

  for (int ix = 0; ix < preview_foot_steps_.size(); ix++)
  {
    robotis_engineer_online_walking_module_msgs::Step2D step;
  ...
  if (humanoid_footstep_client_.call(get_step))
  {
    if (get_step.response.result)
    {
      for (int ix = 0; ix < get_step.response.footsteps.size(); ix++)
      {
        ...
        {
          foot_type = robotis_engineer_online_walking_module_msgs::Step2D::RIGHT_FOOT_SWING;
          foot_string = "right";
        }
        else if (type == humanoid_nav_msgs::StepTarget::left)
        {
          foot_type = robotis_engineer_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING;
          foot_string = "left";
        }
        else
          foot_type = robotis_engineer_online_walking_module_msgs::Step2D::STANDING;
      ...
      if(preview_foot_types_[preview_foot_types_.size() - 1] == robotis_engineer_online_walking_module_msgs::Step2D::RIGHT_FOOT_SWING)
      {
        preview_foot_steps_.push_back(target_l_foot_pose);
        preview_foot_types_.push_back(robotis_engineer_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING);
        preview_foot_steps_.push_back(target_r_foot_pose);
        preview_foot_types_.push_back(robotis_engineer_online_walking_module_msgs::Step2D::RIGHT_FOOT_SWING);
        preview_foot_steps_.push_back(target_l_foot_pose);
        preview_foot_types_.push_back(robotis_engineer_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING);
      }
      else if(preview_foot_types_[preview_foot_types_.size() - 1] == robotis_engineer_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING)
      {
        preview_foot_steps_.push_back(target_r_foot_pose);
        preview_foot_types_.push_back(robotis_engineer_online_walking_module_msgs::Step2D::RIGHT_FOOT_SWING);
        preview_foot_steps_.push_back(target_l_foot_pose);
...
void QNode::visualizePreviewFootsteps(bool clear)
{
  ...
  for (int ix = preview_foot_types_.size() - 1; ix >= 0; ix--)
  {
    ...
    if (!clear)
    {
      ...
      // set foot step color
      if (preview_foot_types_[ix] == robotis_engineer_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING)  // left
      {
      ...
      else if (preview_foot_types_[ix] == robotis_engineer_online_walking_module_msgs::Step2D::RIGHT_FOOT_SWING)  //right
      {
...
// Preview walking
void QNode::sendFootStepCommandMsg(robotis_engineer_online_walking_module_msgs::FootStepCommand msg)
{
  foot_step_command_pub_.publish(msg);
  log( Info , "Send Foot Step Command Msg" );
}

void QNode::sendWalkingParamMsg(robotis_engineer_online_walking_module_msgs::WalkingParam msg)
{
...
void QNode::parseIniPoseData(const std::string &path)
{
  ...
  robotis_engineer_online_walking_module_msgs::JointPose msg;
...
void QNode::sendJointPoseMsg(robotis_engineer_online_walking_module_msgs::JointPose msg)
{
