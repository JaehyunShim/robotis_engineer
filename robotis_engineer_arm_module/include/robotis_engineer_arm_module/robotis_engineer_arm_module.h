...
#ifndef ROBOTIS_ENGINNER_ARM_MODULE_H_
#define ROBOTIS_ENGINNER_ARM_MODULE_H_
...
private:
  ...
  void setArmJointCallback(const sensor_msgs::JointState::ConstPtr &msg);
  void setArmJointOffsetCallback(const sensor_msgs::JointState::ConstPtr &msg);
  void setArmScanCallback(const std_msgs::String::ConstPtr &msg);

  void queueThread();
  void jointTraGeneThread();
  void setArmJoint(const sensor_msgs::JointState::ConstPtr &msg, bool is_offset);
  bool checkAngleLimit(const int joint_index, double &goal_position);
  void generateScanTra(const int arm_direction);
...
#endif /* ROBOTIS_ENGINNER_ARM_MODULE_H_ */
